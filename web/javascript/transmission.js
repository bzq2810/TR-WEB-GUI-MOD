...
Transmission.prototype =
{
	...
	****/
	...
	initialize: function()
	{
		...
		$('#toolbar-update-tracker').click($.proxy(this.updateSelectedClicked,this));
        ...
        $('#statistics-button').click($.proxy(this.showStatsDialog,this));
		...
		e.toolbar_update_button     = $('#toolbar-update-tracker')[0];
		this.elements = e;
	...
	initializeSettings: function()
	{
		...
		// if (this.isMenuEnabled)
		{
			...
			$('#sort_by_mobile_' + this[Prefs._SortMethod]).selectMenuItem();
			if (this[Prefs._SortDirection] === Prefs._SortDescending) {
				$('#reverse_sort_order').selectMenuItem();
				$('#reverse_sort_order_mobile').selectMenuItem();
			}
		}
	...
	createSortMenu: function() {
		$("#sort_super_menu").transMenu({
			open: function() { $("#sort-menu").addClass("selected");},
			close: function() { $("#sort-menu").removeClass("selected"); },
			select: $.proxy(this.onMenuSortClicked, this)
		});
		$("#sort-menu").click(function(event) {
			$("#sort_super_menu").transMenu("open");
		});
	},
	/****
	*****
	...
	****/
	...
	openTorrentClicked: function(ev) {
		if (this.isButtonEnabled(ev)) {
			...
			if (isMobileDevice && this.inspectorIsVisible()) {
	...
	onMenuClicked: function(event, ui)
	{
		...
		if (ui.group == 'sort-mode')
		{
			...
			$('#' + id.replace(/by_/, 'by_mobile_')).selectMenuItem();
		...
		else switch (id)
		{
			...
			case 'reverse_sort_order':
				if (element.menuItemIsSelected()) {
					...
					$('#reverse_sort_order_mobile').deselectMenuItem();
				} else {
	...
	onMenuSortClicked: function(event, ui)
	{
		...
		{
		    element.selectMenuItem();
			$('#' + id.replace(/_mobile/, '')).selectMenuItem();
			this.setSortMethod(id.replace(/sort_by_mobile_/, ''));
		}
		else switch (id)
		{
		    case 'reverse_sort_order_mobile':
			    if (element.menuItemIsSelected()) {
				    dir = Prefs._SortAscending;
					element.deselectMenuItem();
					$('#' + id.replace(/_mobile/, '')).deselectMenuItem();
				} else {
				    dir = Prefs._SortDescending;
					element.selectMenuItem();
					$('#' + id.replace(/_mobile/, '')).selectMenuItem();
	...
	***/
	...
	updateStatusbar: function()
	{
		...
		// visible torrents
		$('#filter-count').text( fmt.countString('','',this._rows.length ) );
	},
	...
	updateButtonStates: function()
	{
		...
		this.calculateTorrentStates(function(s) {
			...
			tr.setEnabled(e.toolbar_update_button, s.sel > 0);
		});
	},
